...
public class LeaseDAO {
	...
	public JSONArray select(int lease_id){
				...
				//System.out.println("list:"+leases);
				return this.searchUtil(leases);
		}
	...
	public boolean removeLease(int lease_id){
		try {
			this.hibernateTemplate.bulkUpdate("update Lease l set l.state='3' where l.id='"+lease_id+"'");
			return true;			
		} catch (Exception e) {
			e.printStackTrace();
			return false;
				...
				if(leaseList != null && leaseList.size() > 0){
					...
					if(result.size() == 1) {
						...
						if (result3.size() == 0) {
							...
							UnitChange testChange = result3.get(0);//代表读出一条记录来赋给testChange
